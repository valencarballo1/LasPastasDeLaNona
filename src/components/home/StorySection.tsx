...
export function StorySection({ image }: { image: ResolvedSiteImage }) {
  ...
  // de la imagen dejaría media pantalla vacía (ver /admin/imagenes).
  const hasPhoto = Boolean(image.src);

  return (
    <section className="bg-warm-white py-16 sm:py-20">
      <Container className={cn("grid items-center gap-10", hasPhoto && "lg:grid-cols-2 lg:gap-12")}>
        {hasPhoto ? (
          <Reveal>
            <SiteImage
              image={image}
        ...
        <Reveal delay={hasPhoto ? 100 : 0} className={cn(!hasPhoto && "mx-auto max-w-2xl text-center")}>
          ...
          <LinkButton href={routes.nosotros} variant="primary" size="md" className="mt-7">
